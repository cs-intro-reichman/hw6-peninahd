...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				flippedHoriz[i][j] = image[i][image[i].length - j -1];
			}
		}
		return flippedHoriz;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		// width = image[0].length (# of columns), height = image.length (number of rows)
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int pixelHeight = (int) (i * scaleHeight);
				int pixelWidth = (int) (j * scaleWidth);
				scaledImage[i][j] = image[pixelHeight][pixelWidth];
			}
		}
		return scaledImage;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 0; i <= n; i++) {
			...
			Color[][] imageMorph = new Color[source.length][source[0].length];
			imageMorph = blend(source, target, alpha);
